...
import { ConceptLearningDetailDialog } from '@/components/admin/ConceptLearningDetailDialog';

// 개념학습 학생 항목 타입
interface ConceptStudent {
  id: number;
  name: string;
  grade: string;
...
export default function ConceptLearningPage() {
  ...
  const [students, setStudents] = useState<ConceptStudent[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchStudents = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/admin/concept-learning?academy_id=${user.academy_id}`);
        ...
        toastRef.current({ title: '오류', description: '개념학습 데이터를 불러오지 못했습니다.', type: 'error' });
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-gray-900">개념학습 관리</h1>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[100px]">학습 회수</TableHead>
                  <TableHead className="w-[100px]">문제풀이</TableHead>
                    ...
                    <TableRow key={student.id}>
                      ...
                      <TableCell className="text-sm font-semibold">
                        {student.totalQuestions > 0 ? (
                          <span
                            className={
                              student.accuracyRate < 80 ? 'text-red-600' : 'text-green-600'
                            }
                          >
