...
export function AdminSidebar({ className }: AdminSidebarProps) {
  ...
  // RAG 관리 서브메뉴들
  const ragSubMenus = useMemo(() => [
    { name: '파일관리', href: '/admin/rag/files', icon: FileText },
    { name: '추천주제', href: '/admin/rag/recommended-topics', icon: FileJson },
    { name: '로그보기', href: '/admin/rag/generation-logs', icon: History },
    { name: '동기화로그', href: '/admin/rag/sync-logs', icon: RefreshCw }
  ], []);
  ...
  useEffect(() => {
    setExpandedItems(prev => {
      ...
      const hasActiveRagSubMenu = ragSubMenus.some(subItem => subItem.href === pathname);
      if (hasActiveRagSubMenu && !prev.includes('RAG관리')) {
        newExpanded.push('RAG관리');
        updated = true;
      }
  ...
  }, [pathname, settingsSubMenus, learningSubMenus, contentsSubMenus, ragSubMenus, statisticsSubMenus]);
          ...
          <div key={category.title}>
            ...
            <div className="space-y-1">
              {category.items.map((item) => {
                ...
                const isRagManagement = item.name === 'RAG관리';
