...
## 코드 수정 후 필수 워크플로우 (최우선!)

코드 수정 완료 후 **절대 바로 배포/커밋 제안 금지**.

```
coder → code-reviewer → project-builder → agent-browser(화면확인) → 배포제안
```

이 순서를 건너뛰면 안 됨!

### 화면확인 (agent-browser) 규칙
- 변경된 코드에서 **테스트 항목을 자동으로 도출**하여 진행
- 테스트 항목 예시:
...
- **OS**: Windows 11, **셸**: Git Bash, **루트**: `c:\Source\AcademyAdmin`
- Bash 명령어 실행 시 Windows 경로(`c:\...`) 그대로 사용 가능, 단 한글/공백 포함 경로는 반드시 **큰따옴표**로 감싸기
- `NUL` 대신 `/dev/null` 사용 (Git Bash 환경)
